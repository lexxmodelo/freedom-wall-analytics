"""Centralized logging for the topic_modeling pipeline.

Mirrors preprocessing/preprocessing/logging_setup.py. Logs go to stderr and an
optional per-run file; a separate `log_action()` helper appends Markdown entries
to action_log.md so the audit trail and console logs stay in sync.
"""
...
def setup_logger(name: str, log_dir: Path | None = None) -> logging.Logger:
    ...
    if log_dir is not None:
        ...
        fh = logging.FileHandler(log_dir / f"topic_modeling_{stamp}.log", encoding="utf-8")
        fh.setFormatter(fmt)
