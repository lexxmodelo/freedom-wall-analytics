...
Patterns copied from preprocessing/preprocessing/io_utils.py to keep the topic_modeling
package self-contained (no sys.path hacks, no cross-package imports). All file ops are
UTF-8 explicit because the corpus contains non-ASCII characters.
"""
...
def load_yaml(path: Path) -> Any:
    ...
    import yaml  # local import: only needed when YAML is actually loaded
    with path.open("r", encoding="utf-8") as f:
        return yaml.safe_load(f)


def sha256_text(text: str) -> str:
    """Return the hex sha256 digest of a UTF-8 string. Used for prompt-drift detection."""
